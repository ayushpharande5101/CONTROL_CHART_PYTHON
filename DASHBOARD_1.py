...
def show_chart(f_date, t_date, batch):
    ...
    if selected_chart_type == 'SPC Chart':
        ...
        plt.axhline(y=UCL, color='r', linestyle='--', label='UCL')
        plt.axhline(y=LCL, color='r', linestyle='--', label='LCL')
        for label, value in [('UCL', UCL), ('LCL', LCL), ('X-bar', X_bar)]:
        ...
        sql_query = ("SELECT SR_NO,DATETIME,BATCH,OC_POINTS FROM Control_chart.dbo.Histogram  "
                     "WHERE DATETIME BETWEEN ? AND ? OR BATCH = ?")
...
with m_side:
    ...
    t_date = str(To_date)
    batch_n = s_option
    batch = int(batch_n.replace('Batch ', 'SPC Chart'))
